...
<div class="modal">
    <a href="${pageContext.request.contextPath}/account?others_user_name=null"><i class="fas fa-arrow-left"></i></a>
    <form action="${pageContext.request.contextPath}/updateAccount" method="post" enctype="multipart/form-data">
        ...
        <ul>
            ...
            <li><textarea name="status" id="" cols="30" rows="4"><c:if test="${status != null}">${status}</c:if><c:if test="${status == null}">${User.getStatus()}</c:if></textarea>
                <i class="fas fa-exclamation-circle dacbiet" id="icon_status"></i></li>
            <li>
                <button class="btnSubmit" name="submitRegister" type="submit">Cập nhật</button>
            </li>
        </ul>
