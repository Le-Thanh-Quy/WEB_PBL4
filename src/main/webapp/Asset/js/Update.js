function loadFind() {
    ...
    if (type.value == 'Province') {
        xhttp.onload = function () {
            ...
            var html = '<form action="DeleteProvince" method="post"><table><td></td><td></td><td></td> <td></td><td></td><td><input style="opacity: 0;" id="deleteBtn" type="submit" value="Xóa" ></td> ';
            html += '<tr><th>ID</th>' +
            ...
            for (const x of myObj) {
                html += ' <tr class="tag' + myObj.indexOf(x) % 2 + '"><td>' + x.matp + '</td>' +
                    ...
                    '                                class="fas fa-eye"></i> Chi tiết</button></td> ' +
                    '<td><input onchange="Chosse(this)" id="cb" type="checkbox" name="cb" value="' + x.matp + '"/></td> </tr>';
            }
            html += '</table></form>';
        ...
        xhttp.onload = function () {
            const myObj = JSON.parse(this.responseText);
            var html = '<form action="DeleteDistrict" method="post"><table><td  class="back"><a href="getProv"><i class="fas fa-angle-double-left"></i></a></td><td></td> <td></td><td></td> <td><input type="hidden" name="IDProvince" value="' + IDDistrict + '"></td><td><input  id="deleteBtn" type="submit" value="Xóa" ></td> ';
            html += '<tr><th>ID</th>' +
            ...
            for (const x of myObj) {
                html += '<tr class="tag' + myObj.indexOf(x) % 2 + '"><td>' + x.maqh + '</td>' +
                    ...
                    '                                class="fas fa-eye"></i> Chi tiết</button></td>' +
                    '<td><input type="checkbox" name="cb" value="' + x.maqh + '"/></td></tr>';
            }
            html += '</table></form>';
            table.innerHTML = html;
        }
...
function loadDistrict(IDProv) {
    ...
    xhttp.onload = function () {
        ...
        var html = '<form action="DeleteDistrict" method="post"><table><td  class="back"><a href="getProv"><i class="fas fa-angle-double-left"></i></a></td><td></td><td></td><td></td> <td><input type="hidden" name="IDProvince" value="' + IDProv + '"></td><td><input   id="deleteBtn" type="submit" value="Xóa" ></td> ';
        html += '<tr><th>ID</th>' +
            '<th>Tên Quận/Huyện</th>' +
            '<th>Kiểu</th>' +
            '<th>Cập nhật thông tin</th> ' +
